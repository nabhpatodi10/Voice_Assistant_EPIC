                    ...
                    def Main_Function():
                        ...
                        def listentome():
                            ...
                            try:
                                ...
                                try:
                                    cursor.execute("SELECT Path FROM applications where Applications='Google Chrome';")
                                    chrome_path=str(cursor.fetchall()[0][0])+" %s"

                                except:
                                    t="Sir please give me the path of Google Chrome as it hasn't been fed into my Database"
                                    talktome(t)
                                    file = fd.askopenfile()

                                    if file: 
                                        p=file.name
                                        PATH=f"INSERT INTO Applications VALUES ('Google Chrome', '{p}');"
                                        cursor.execute(PATH)
                                        db_connection.commit()
                                ...
                                elif "notepad" in task or "Notepad" in task:
                                    t="Opening Notepad"
                                    talktome(t)
                                    cursor.execute(f"UPDATE chat_history SET Chat_EPIC='{t}' WHERE Date='{DATE}' AND Chat_User='{task}' AND Chat_EPIC IS NULL;")
                                    db_connection.commit()
                                ...
                                elif "cmd" in task or "CMD" in task or "Command Prompt" in task or "command prompt" in task or "Command prompt" in task:
                                    t="Opening Command Prompt"
                                    talktome(t)
                                    cursor.execute(f"UPDATE chat_history SET Chat_EPIC='{t}' WHERE Date='{DATE}' AND Chat_User='{task}' AND Chat_EPIC IS NULL;")
                                    db_connection.commit()

                                    sp.Popen("C:/WINDOWS/system32/cmd")
                                
                                elif "Calculator" in task or "calculator" in task:
                                    t="Opening Calculator"
                                    talktome(t)
                                    cursor.execute(f"UPDATE chat_history SET Chat_EPIC='{t}' WHERE Date='{DATE}' AND Chat_User='{task}' AND Chat_EPIC IS NULL;")
                                    db_connection.commit()

                                    sp.Popen("C:/WINDOWS/system32/calc")
                                ...
                                elif "play" in task or "Play" in task or "music" in task or "Music" in task:
